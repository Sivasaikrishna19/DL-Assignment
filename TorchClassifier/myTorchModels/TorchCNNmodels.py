...
def create_vggmodel1(num_classes):
    ...
    # Replace the last layer in the VGG classifier group of layers
    n_inputs = vgg16.classifier[6].in_features
    vgg16.classifier[6] = nn.Linear(n_inputs, num_classes)
...
def create_vggcustommodel(num_classes, config, batch_norm=True):
    # Create VGG layers based on the provided config and whether to include batch normalization
    features = get_vgg_layers(config, batch_norm=batch_norm)
    model = nn.Sequential(
        features,
